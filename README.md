...
- ✅ **图片接收功能**：用户发送图片时，自动保存到 cache 文件夹，并可用于需要图片参数的 API 调用
- ✅ **Cache 管理**：自动清理过期缓存（1小时），容量超限时自动清理最旧的文件
...
| `enable_llm_rewrite` | bool | 是否启用 LLM 回复改写（开启后，API 返回的文本会由 LLM 进行人性化改写，让回复更像真人） | `false` |
| `enable_image_receive` | bool | 是否启用图片接收功能（开启后，用户发送的图片会自动保存到 cache 文件夹，并可用于需要图片参数的 API 调用） | `true` |
| `cache_size_limit` | int | Cache 文件夹的容量限制（单位：MB，超过限制会自动清理最旧的缓存文件） | `100` |
...
插件支持接收用户发送的图片，并用于需要图片参数的 API 调用。图片参数分两种类型：

- **`image_file`（本地图片）**：直接发送图片即可，Bot 会自动处理
- **`image_url`（图片URL）**：需要图片的公网URL，Bot 会尝试获取平台图片URL；若获取失败，请直接发送图片链接
...
**注意**：对于需要 `image_url`（图片URL）的 API，Bot 会尝试获取平台（QQ/Telegram等）的原始图片URL。如果平台未暴露图片URL，请**直接发送图片链接**（以 `http://` 或 `https://` 开头）。

```
用户： [发送消息："动漫搜番" + 图片]
Bot： [调用动漫搜番 API，使用消息中的图片]

用户： 百度OCR https://example.com/image.jpg
Bot： [调用百度OCR API，使用图片URL]
```

#### 方式2：分开发送（仍支持）

1. **发送图片给 Bot**：直接发送图片（不是图片链接）
2. **Bot 确认接收**：Bot 会回复「图片已接收！现在你可以输入需要图片的 API 关键词来调用 API。」
3. **输入关键词**：发送需要图片参数的 API 关键词
4. **Bot 调用 API**：Bot 会使用你刚才发送的图片作为 API 参数，调用对应的 API

#### 示例（方式1：一起发送）

```
用户： [发送消息："动漫搜番" + 图片]
Bot： [调用动漫搜番 API，使用消息中的图片进行识别]
```
...
用户： [发送图片]
Bot： 📷 图片已接收！
      现在你可以输入需要图片的 API 关键词来调用 API。
      提示：输入「api查询」查看所有可用关键词。

用户： 人脸识别
Bot： [调用人脸识别 API，使用你刚才发送的图片]
```
...
- 📷 `image_file`：**直接发送图片给 Bot**（本地图片，Bot 自动转 base64）
- 🖼️ `image_url`：**发送图片链接**（以 `http://` 或 `https://` 开头），或发送图片并等待 Bot 获取平台URL
- 🔊 `audio_url`：**发送音频链接**
- 🎬 `video_url`：**发送视频链接**
- 🔗 `url`：**发送链接地址**
...
- **自动清理**：每次启动插件时，会自动清理超过 1 小时的缓存文件
- **容量限制**：当 cache 文件夹超过设定容量（默认 100MB）时，会自动清理最旧的文件
- **配置容量**：在插件配置中修改 `cache_size_limit`（单位：MB）
...
图片类 API（如动漫搜番、OCR 等）需要提供图片的 **URL 链接**，不是直接发送图片文件。
...
PS:这个插件是我第一次纯使用Agent跑的所有代码(也是第一次开发)，对于Astrbot和插件的写法规范我并不是很熟练，我唯一只手动修改了关键词（keywords）和api接口的映射，并没有做其他的检查，可能存在很多bug和不足的地方，欢迎和我反馈或者有更好的建议哪怕是聊天都可以联系我。
