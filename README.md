...
AstrBot 插件，聚合调用 TinyAPI 的 250+ API调用接口，支持关键词快捷调用、智能参数提示，图片/音频/视频自动识别并直接发送。
...
- ✅ **250+ API调用接口**：覆盖出行、AI、游戏、工具、生活、娱乐等多个领域
  ...
  "sites": [
    {
      ...
      "description": "TinyAPI 250+ API调用接口平台",
      "enabled": true
    }
  ]
}
...
- 🐛 **修复群聊 @ 机器人时关键词？查询功能失效**：过滤 `[At:...]` 标记，避免 QQ 平台 @ 消息导致关键词匹配失败
- 📝 更新插件名称为 TinyAPI（原：TinyAPI聚合插件）
- 📝 精简 README 功能特性：删除已过时的"图片接收功能"、"Cache 管理"、"媒体+文字同时输出"描述
- 📝 更新安装方法：不指定具体版本号，引导用户下载最新版本
...
- [TinyAPI](https://api.tinyaii.top)：提供 250+ API调用接口
